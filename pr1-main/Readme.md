...
This project focuses on **socket programming** and **multi-threading using pthreads**. 
You do **not** need to use `fork()` anywhere in this project (Warm-up, Part 1, or Part 2). 
While using `fork()` in the Warm-up exercises may not affect your grade, **Parts 1 and 2 
require the use of pthreads for concurrency, and using `fork()` instead will result in 
lost points.**
...
> [!NOTE]
> For both the client and the server, your code **must** follow a boss-worker thread 
pattern using the **pthreads** library. This design pattern will be graded. 
See the [Important Note on Process vs. Thread Model](#important-note-on-process-vs-thread-model) 
above regarding the prohibition on using `fork()`.
